...
OpenAI Codex assisted with PS analysis, literature research, concept selection, mathematics, code, tests, interface work, documentation, and QA. The optional runtime endpoint attempts Grok 4.1 Fast through Vercel AI Gateway and otherwise serves a repository-authored deterministic co-review; the interface names the actual responder, and neither response is experiment evidence. Runtime vectors are deterministic and synthetic. No external datasets, model weights, generated media, copied template, external font, or proprietary code are shipped. Repository-authored code and prose are MIT-licensed; cited papers are linked rather than redistributed.
