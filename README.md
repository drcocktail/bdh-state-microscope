...
| Grok research interlocutor | Challenge or teach one selected deterministic trace | Optional model-generated commentary through a server-only Vercel AI Gateway endpoint; never an oracle or evidence source |
...
No environment variables, API keys, accounts, model weights, remote services, or network calls are required for the complete deterministic microscope. The optional essay companion calls a server-only Vercel AI Gateway endpoint; Vercel deployment OIDC authenticates it without shipping a browser secret. If that endpoint is unavailable, every experiment, verdict, test, and source remains usable.
...
OpenAI Codex assisted with problem-statement analysis, literature research, concept selection, mathematics, implementation, testing, interface work, documentation, and QA. The submitting team must run and defend the mechanism and independently verify the cited primary work. At runtime, Grok 4.1 Fast can generate optional bounded commentary from a declared numeric trace through Vercel AI Gateway. That text is labelled as stochastic interpretation, is never used as experiment evidence, and is not committed as a reproduced result. No external generated media or proprietary code is shipped.
...
- Runtime model service: optional Grok 4.1 Fast via Vercel AI Gateway; server-side OIDC, finite structured inputs, capped output, cached responses.
